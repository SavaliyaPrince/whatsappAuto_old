...
class SendMessageController extends GetxController {
  ...
  String getCountryCode =  '+1';

  // Future<String> getCountryNumber() async {
  //   /// more than 10  digits and start with 00 remove 00 only
  //   final String mobileNumber = phoneController.text;
  ...
  //     }
  //   } else {
  //     if (prefs.getString("countryCode") != "") {
  //       countryCode.value = prefs.getString("countryCode") ?? "";
  //       return countryCode.value + mobileNumber;
  //     }
  //   }
  //   return mobileNumber;
    ...
    if (phoneController.text != "") {
      try {
        if (Platform.isIOS) {
          ...
          log("Mobile Number $mobileNumber");
          log("Mobile Number 000000 ${getCountryCode}");
          final redirectUrl = "https://wa.me/$getCountryCode$mobileNumber?text=${Uri.parse(message.replaceAll(":", ""))}";
          //await launch(redirectUrl);
          log("redirectUrl --0--> $redirectUrl");
          ...
          final redirectUrl = "https://wa.me/$mobileNumber?text=${Uri.parse(message.replaceAll(":", ""))}";
          log("redirectUrl --1-- > $redirectUrl");
          //await launch(redirectUrl);
          if (await canLaunch(redirectUrl)) {
            log("------1-----");
            await launch(redirectUrl, forceSafariVC: false);
  ...
  String formatMillisecondToDate(int milliSecond) {
    ...
    final String formattedDate = DateFormat('d/M/yy h:mm a').format(date).toString();
    String dateTime = "";
  ...
  String formatDuration(int seconds) {
    if (seconds != null && seconds != 0) {
      ...
      final seconds1 = (seconds % 3600).truncate();
      final int minutes = (seconds1 / 60).truncate();

      final String hoursStr = (hours).toString().padLeft(1, '0');
      final String minutesStr = (minutes).toString().padLeft(1, '0');
      final String secondsStr = (seconds1 % 60).toString().padLeft(0, '');
  ...
  //   try {
  //     final prefs = await SharedPreferences.getInstance();
  //     countryCodeModel.value = await SendMessageService.getCountryCode();
  //     // countryCode.value = countryCodeModel.value.countryCode ?? '';
  //     final GetCodeModel getCode = allData.firstWhere(
  //         (element) => element.code == countryCodeModel.value.countryCode);
  //     countryCode.value = getCode.dialCode ?? '';
  //     await prefs.setString("countryCode", getCode.dialCode ?? '');
  //   } catch (e, st) {
  //     log("Error Message: $e : $st");
  //   }
  // }

  // static List<GetCodeModel> fromList(dynamic json) {
  //   final List<GetCodeModel> tempList = [];
  //
  //   json.forEach((element) {
  //     tempList.add(GetCodeModel.fromJson(element));
  //   });
  //
  //   return tempList;
  // }
...
// List<GetCodeModel> allData = fromList([
